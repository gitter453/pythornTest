...
#화면 타이틀 설정
pygame.display.set_caption("retropangpang") #게임 이름
...
while running:
    ...
    for event in pygame.event.get():  
        ...
        if event.type == pygame.KEYDOWN: 
            ...
            if event.key == pygame.K_RIGHT: #캐릭터를 왼쪽으로
                character_to_x += character_speed
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        #공이 좌/우 벽에 닿았을 때 공 이동위치 변경 (공이 튕겨나오는 효과)
        if ball_pos_x < 0 or ball_pos_x > screen_width - ball_width: 
            ball_val["to_x"] = ball_val["to_x"] * -1

        #공이 스태이지에 튕겨서 올라가는 처리
        if ball_pos_y >= screen_height - stage_height - ball_height:
            ball_val["to_y"] = ball_val["init_spd_y"] 
        else: #그 외의 모든 경우에는 속도를 증가(포물선효과)
            ball_val["to_y"] += 0.5
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            continue #안쪽 for 문 조건이 맞지 않으면 continue. 바깥 for 문 계속수행
        break #안쪽 for 문에서 break를 만나면 여기로 진입. 2중 for문을 한번에 탈출 
...
#게임종료메세지
msg = game_font.render(game_result, True, (255, 255, 0)) #노란
msg_rect = msg.get_rect(center=(int(screen_width / 2) ,int(screen_height / 2)))
screen.blit(msg,msg_rect)
pygame.display.update()

pygame.time.delay(2000) #글자출력을 위한 2초대기
